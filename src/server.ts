...
app.post(
  '/chat',
  async (req: Request<object, object, ChatRequest>, res: Response) => {
    ...
    console.log(`[${new Date().toISOString()}] Request: ${message.substring(0, 50)}...`);

    try {
      const { streamAgent } = await import('./workflow/orchestrator.js');
